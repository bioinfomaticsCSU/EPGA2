...
// # of empty bucket
static long long nbEmptyBucket = 0;
...
void sortentry(string namefile, const int k, const int m, bool create_buckets, bool m_mer_count){
    ...
    while (1)
    {
        ...
        if (kmer == "")
            break;
		//no more consider the revc of kmer
		//string ckmer = reversecompletment(kmer);
        if (m_mer_count)
        {
			nbKmer++;
            count_m_mers(kmer, 2*m, k);
			//no more count the mmer of kmer's revc 
			//count_m_mers(ckmer,2*m,k);
            continue;
        }

        int middlemin, leftmostmin, rightmostmin, min;
        
		//no more minimiser value of kmer's revc
		...
		middlemin = minimiserv(kmer.substr(1,k-2),2*m);
		leftmostmin = minimiserv(kmer.substr(0,2*m),2*m);
		rightmostmin=minimiserv(kmer.substr(kmer.size()-2*m,2*m),2*m);
		...
		/*
		int rcmiddlemin, rcleftmostmin, rcightmostmin, rcmin;
		middlemin=minimiserrc(kmer.substr(1,k-2),2*m);
        leftmostmin=minimiserrc(kmer.substr(0,2*m),2*m);
        rightmostmin=minimiserrc(kmer.substr(kmer.size()-2*m,2*m),2*m);
		*/
    ...
    if (create_buckets)
		...
		cout << "Total # of kmer: " <<nbKmer << endl;
}
...
void createoutfile(const char *namein,const char *nameout,const int k,const int m){
    ...
    sortentry(namein,k,m, false, true);//count minimiser
	...
	// show the # of nonempty buckets
	cout << pow(4,2*m)-nbEmptyBucket <<" nonempty bucket(s) created" << endl;
